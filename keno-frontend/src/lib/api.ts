...
export async function createTicket(params: { roundId: string; numbers: number[] }): Promise<Ticket> {
  ...
  const res = await fetch(`${API_BASE}/tickets?${qs.toString()}`, { method: 'POST' })
...
export async function postDraw(roundId: string): Promise<{
  ...
  const res = await fetch(`${API_BASE}/drawnings?round_id=${encodeURIComponent(roundId)}`, { method: 'POST' })
